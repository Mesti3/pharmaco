...
namespace pharmaco.service.app
{
    class service_controller
    {
        ...
        private List<int> displayed_order_ids;
        private List<int> my_order_in_progress_ids;
        public service_controller(taksbar_icon tbi)
        {
            this.tbi = tbi;
            var client_id = ConfigurationManager.AppSettings["ClientID"];
            data = new DataController(client_id);
            error_counter = 0;
            displayed_order_ids = new List<int>();
            my_order_in_progress_ids = new List<int>();
        }

        public void load_new_orders()
        {
            try
            {
                var orders = data.GetAllOrders(DateTime.Now.AddHours(-1), DateTime.Now,null, orderstate.created).Where(x=>!displayed_order_ids.Contains(x.id)).ToList();

                if (orders.Count > 0)
                {
                    show_orders_in_baloon(orders, false);
                    displayed_order_ids.AddRange(orders.Select(x => x.id));
        ...
        private void show_orders_in_baloon(List<order> orders, bool stay_open)
        {
        ...
        internal void open_balloon()
        {
            try
            {
                var orders = data.GetAllOrders(DateTime.Now.AddHours(-1), DateTime.Now, null, orderstate.created);
                if (my_order_in_progress_ids.Count > 0)
                    orders.AddRange(data.GetAllOrders(DateTime.Now.AddHours(-1), DateTime.Now, my_order_in_progress_ids));
                if (orders.Count > 0)
                {
                    show_orders_in_baloon(orders, false);
                }
        ...
        private void balloon_cancel_clicked(notification_baloon_element obj)
        {
            try
            {
                ...
                my_order_in_progress_ids.Remove(obj.order.id);
            }
            catch (Exception ex)
            {
...
#nullable disable
        ...
        private void balloon_process_clicked(notification_baloon_element obj)
        {
            ...
            try
            {
                ...
                if (o.Item1 == orderstate.created)
                {
                    ...
                    my_order_in_progress_ids.Add(obj.order.id);
                }
                obj.update_state();
            }
            catch (Exception ex)
        ...
        private void show_default_error_ballon()
        {
            error_baloon balloon = new error_baloon();
            tbi.ShowCustomBalloon(balloon, PopupAnimation.Slide);
        }
